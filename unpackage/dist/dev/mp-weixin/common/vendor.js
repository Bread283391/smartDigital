...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__8AD0D6F",
    appName: "smartDigital",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
function ownKeys$1(target) {
  track$1(target, "iterate", isArray(target) ? "length" : ITERATE_KEY$1);
...
const shallowReactiveHandlers = /* @__PURE__ */ extend({}, mutableHandlers$1, {
  get: shallowGet,
  set: shallowSet
});
const shallowReadonlyHandlers$1 = /* @__PURE__ */ extend({}, readonlyHandlers$1, {
...
function getTargetType$1(value) {
  return value[
    "__v_skip"
    /* ReactiveFlags.SKIP */
  ] || !Object.isExtensible(value) ? 0 : targetTypeMap$1(toRawType(value));
}
...
let RefImpl$1 = class RefImpl {
  ...
  set value(newVal) {
    const useDirectValue = this.__v_isShallow || isShallow$2(newVal) || isReadonly$1(newVal);
...
function flushPreFlushCbs(seen, i = isFlushing$1 ? flushIndex$1 + 1 : 0) {
  {
...
function injectHook(type, hook, target = currentInstance$1, prepend = false) {
  if (target) {
    ...
    const apiName = toHandlerKey((ErrorTypeStrings$1[type] || type.replace(/^on/, "")).replace(/ hook$/, ""));
...
const hasSetupBinding$1 = (state, key) => state !== EMPTY_OBJ && !state.__isScriptSetup && hasOwn$1(state, key);
const PublicInstanceProxyHandlers$1 = {
  ...
  has({ _: { data, setupState, accessCache, ctx, appContext, propsOptions } }, key) {
    let normalizedProps;
    return !!accessCache[key] || data !== EMPTY_OBJ && hasOwn$1(data, key) || hasSetupBinding$1(setupState, key) || (normalizedProps = propsOptions[0]) && hasOwn$1(normalizedProps, key) || hasOwn$1(ctx, key) || hasOwn$1(publicPropertiesMap$1, key) || hasOwn$1(appContext.config.globalProperties, key);
  },
...
function callHook$1(hook, instance, type) {
  callWithAsyncErrorHandling$1(isArray(hook) ? hook.map((h) => h.bind(instance.proxy)) : hook.bind(instance.proxy), instance, type);
}
...
const queuePostRenderEffect$2 = queuePostFlushCb$1;
...
function createInvoker(initialValue, instance) {
  const invoker = (e2) => {
    ...
    const invoke = () => callWithAsyncErrorHandling$1(patchStopImmediatePropagation(e2, eventValue), instance, 5, args);
...
function doWatch(source, cb, { immediate, deep, flush, onTrack, onTrigger } = EMPTY_OBJ) {
  ...
  const effect = new ReactiveEffect2(getter, scheduler);
  {
    effect.onTrack = onTrack;
    effect.onTrigger = onTrigger;
  }
